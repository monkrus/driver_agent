...
def get_driver_response():
    ...
    response = random.choices(['yes', 'no'], weights=[0.5, 0.5], k=1)[0]  # 50% chance of "Yes"
...
def main():
    """Run the driver stress detection loop multiple times until both conditions are met."""
    print("Starting the Driver Stress Detection Demo...")

    # Loop runs indefinitely until both stress is detected and the driver says "yes"
    while True:
        ...
        if stress_detected:
            ...
            if get_driver_response():
                # Both conditions met: stress detected and driver said "yes"
                print("Both conditions met: High stress and 'Yes' response from the driver.")
                
                # Step 5: Play a jazz playlist
                play_jazz_playlist()
                break  # Exit the loop once the conditions are met
            else:
                print("Driver does not want music. Restarting the loop to check again...")
        else:
            print("No stress detected. Restarting the loop to check again...")
